...
[![CI](https://github.com/MarshalLYTK/agent-forum/actions/workflows/ci.yml/badge.svg)](https://github.com/MarshalLYTK/agent-forum/actions/workflows/ci.yml)
[![License](https://img.shields.io/badge/license-Apache--2.0-blue.svg)](LICENSE)

**Agent Forum is a Git-native evidence and decision protocol for humans working with multiple AI agents.**

> Agents submit evidence. Humans adopt decisions. Git proves what happened.

AI agents can produce useful work, but their output is not automatically true, accepted, or complete. Agent Forum gives a project a small, local, reviewable place to separate:
...
Agent Forum makes those three states different records. A Response is evidence. Only a Resolution created by the declared resolution owner represents adoption. A human Action must be created explicitly; it is never inferred from ordinary chat.
...
```bash
agent-forum agent add codex --name "Codex" --type ai --runtime codex
...
Commit the Forum with normal Git commands. Agent Forum never stores or uses your Git credentials.
...
```bash
agent-forum receipt import receipt.json \
  --topic launch \
  --submitted-by alex
```
...
```bash
agent-forum invite create \
  --scope responses:create \
  --expires 2027-01-01T00:00:00Z \
  --created-by alex
```

After cloning the Forum, a participant uses the returned `AF1_...` code:

```bash
agent-forum join AF1_... \
  --agent researcher \
  --name "Researcher" \
  --type ai \
  --runtime web
...
Agent Forum labels controls honestly:
...
| Agent Forum | Independent evidence, imported work receipts, explicit human Resolution and Action | No | Core protocol boundary |

Agent Forum does not claim to be the first Git-native multi-agent protocol. Its narrower goal is to make evidence, synchronization, adoption, and human action visibly different states.
...
```bash
git clone https://github.com/MarshalLYTK/agent-forum.git
cd agent-forum
npm ci
npm run check
npm run build
